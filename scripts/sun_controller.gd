...
func _get_sun_radius() -> float:
	# Use the actual_radius from inherited planet_positioning.gd
...
func _get_node_radius(node: Node3D) -> float:
	...
	if node.has_method("get"):
		var node_actual_radius = node.get("actual_radius")
		if node_actual_radius:
			var node_scale_factor = node.get("scale_factor")
			if node_scale_factor:
				return node_actual_radius * node_scale_factor
